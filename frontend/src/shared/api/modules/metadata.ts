...
export const metadataApi = {
  ...
  getSchemas: (backendId: string): Promise<string[]> =>
    get<{ schemas: string[] }>(`/api/v1/analysis/backends/${backendId}/schemas`)
      .then((data) => data.schemas),

  getTables: (backendId: string, schema: string): Promise<TableInfo[]> =>
    get<{ tables: TableInfo[] }>(`/api/v1/analysis/backends/${backendId}/schemas/${schema}/tables`)
      .then((data) => data.tables),

  getColumns: (backendId: string, schema: string, tableName: string): Promise<ColumnInfo[]> =>
    get<{ columns: ColumnInfo[] }>(`/api/v1/analysis/backends/${backendId}/schemas/${schema}/tables/${tableName}/columns`)
