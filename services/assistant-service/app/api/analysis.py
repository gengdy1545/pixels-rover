...
async def submit_analysis(
    ...
    thread_result = await db.execute(
        select(ConversationThread).where(
            ...
            ConversationThread.user_id == current_user.user_id,
        )
    )
...
async def get_analysis_result(
    ...
    step_result = await db.execute(
        ...
        .where(AnalysisStepRecord.session_id == session.id)
        .order_by(AnalysisStepRecord.sequence_number)
    )
    ...
    if not step_results and isinstance(session.plan_json, dict):
        step_results = session.plan_json.get("steps", []) or []
