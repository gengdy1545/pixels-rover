...
public class QueryController
{
    ...
    public ApiResponse<SubmitQueryResponse> submitQuery(@RequestBody SubmitQueryRequest request)
    {
        ...
        {
            log.error("Upstream query server returned error for submitQuery: {}", e.getMessage());
            return ApiResponse.error(ErrorCode.INTERNAL_ERROR, "Upstream query service error: " + e.getStatusCode());
        }
        catch (WebClientRequestException e)
        {
            log.error("Failed to connect to upstream query server for submitQuery: {}", e.getMessage());
            return ApiResponse.error(ErrorCode.INTERNAL_ERROR, "Upstream query service unavailable");
        }
        catch (Exception e)
        {
            log.error("Unexpected error in submitQuery", e);
    ...
    public ApiResponse<GetQueryStatusResponse> getQueryStatus(@RequestBody GetQueryStatusRequest request)
    {
        ...
        {
            log.error("Upstream query server returned error for getQueryStatus: {}", e.getMessage());
            return ApiResponse.error(ErrorCode.INTERNAL_ERROR, "Upstream query service error: " + e.getStatusCode());
        }
        catch (WebClientRequestException e)
        {
            log.error("Failed to connect to upstream query server for getQueryStatus: {}", e.getMessage());
            return ApiResponse.error(ErrorCode.INTERNAL_ERROR, "Upstream query service unavailable");
        }
        catch (Exception e)
        {
            log.error("Unexpected error in getQueryStatus", e);
            return ApiResponse.error(ErrorCode.INTERNAL_ERROR, e.getMessage());
        }
    }
    ...
    public ApiResponse<GetQueryResultResponse> getQueryResult(@RequestBody GetQueryResultRequest request)
    {
        ...
        {
            log.error("Upstream query server returned error for getQueryResult: {}", e.getMessage());
            return ApiResponse.error(ErrorCode.INTERNAL_ERROR, "Upstream query service error: " + e.getStatusCode());
        }
        catch (WebClientRequestException e)
        {
            log.error("Failed to connect to upstream query server for getQueryResult: {}", e.getMessage());
            return ApiResponse.error(ErrorCode.INTERNAL_ERROR, "Upstream query service unavailable");
        }
        catch (Exception e)
        {
            log.error("Unexpected error in getQueryResult", e);
            return ApiResponse.error(ErrorCode.INTERNAL_ERROR, e.getMessage());
        }
    }
}