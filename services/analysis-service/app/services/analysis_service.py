...
class AnalysisService:
    ...
    async def run_analysis_stream(
        ...
        yield _sse_event(SSEEventType.STATUS_CHANGE, {"status": "received", "session_id": session_id})
        ...
        backend = self.backend_registry.get_for_schema(
            resolved.metrics[0].source_schema
        )
        ...
        await self._finalize(db, session, status, task=task, plan=plan, summary=summary, ctx=ctx)
